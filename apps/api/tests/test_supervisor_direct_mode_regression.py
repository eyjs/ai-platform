...
검증 시나리오:
  ...
  3. 자동 라우팅 흡수(Phase 3 컷오버) — chatbot_id=None 요청은 supervisor가 처리하고,
     supervisor 미배선이면 400으로 명시 거절된다(레거시 오케스트레이터 제거됨).
...
class TestAutoRoutingAbsorbedBySupervisor:
    """Phase 3 컷오버: chatbot_id=None은 supervisor가 처리하고 레거시 경로는 없다."""

    @pytest.mark.asyncio
    async def test_prepare_chat_rejects_none_chatbot_id_with_400(self):
        """직접 모드 헬퍼는 chatbot_id 미지정을 400으로 명시 거절한다(레거시 라우팅 제거)."""
    ...
    @pytest.mark.asyncio
    async def test_full_chat_entrypoint_none_chatbot_id_goes_to_supervisor(self, monkeypatch):
        """chatbot_id=None 전체 /chat 흐름은 supervisor.supervise로 흐른다(직접 경로 미사용)."""
        supervisor = AsyncMock()
        supervisor.supervise.return_value = AgentResponse(answer="자동 라우팅 답변", sources=[])
        state = _make_full_state(supervisor=supervisor)
        ...
        req = ChatRequest(question="아무 질문")  # chatbot_id 생략 -> 자동 라우팅
        request = _make_request(state)

        resp = await chat_module.chat(req, request)

        supervisor.supervise.assert_awaited_once()
        state.agent.execute.assert_not_called()  # 직접 경로 미사용
        assert resp.answer == "자동 라우팅 답변"
