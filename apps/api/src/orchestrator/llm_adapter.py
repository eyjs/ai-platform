...
if TYPE_CHECKING:
    from src.infrastructure.providers.base import OrchestratorLLMConfig
...
class OrchestratorLLM:
    ...
    @classmethod
    def from_config(cls, cfg: "OrchestratorLLMConfig") -> "OrchestratorLLM":
        """팩토리가 해석한 백엔드 설정으로 어댑터를 생성한다 (계층 경계 준수)."""
        return cls(
            provider=cfg.provider, model=cfg.model, api_key=cfg.api_key,
            timeout=cfg.timeout, server_url=cfg.server_url, ollama_host=cfg.ollama_host,
        )
