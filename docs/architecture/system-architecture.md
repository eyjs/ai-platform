...
| C4 | Agent | 12 | LangGraph (deterministic/agentic), Plan-and-Execute, Graph Cache |
| C5 | Workflow | 8 | 절차 기반 대화, Action Step, 세션 영속화 |
...
| C10 | Pipeline | 5 | Parse(DocForge 위임)->Chunk->Embed->Index (비동기 Job Queue) |
...
3개 서브쿼리를 `asyncio.gather`로 동시 실행. RAG neighbor expansion도 `WHERE id = ANY($1)` 단일 IN 쿼리로 통합.
...
| workflow_states | 워크플로우 상태 + 세션 영속화 | Normal (persistent) |
| api_keys | API Key 관리 | Normal |
| request_logs | 요청 로그 | Normal |
| response_cache | 응답 캐시 | Normal |
| feedback | 사용자 피드백 | Normal (30일 auto-purge) |

## Performance Architecture (v0.11.0)

```
[Graph LRU Cache] ─── (profile_id, tool_set) 복합 키, TTL 2h
        │
[Prompt Caching] ─── OpenAI/Anthropic cache_control (system prompt)
        │
[Parallel RAG] ─── asyncio.gather (hybrid search + neighbor expansion)
        │
[SQL-level JSONB Slice] ─── DB에서 최근 N턴만 추출 (Python 전체 로드 제거)
        │
[Explicit Pool Params] ─── pool_size=5, max_overflow=10, recycle=3600
```

## Workflow Engine

```
input step ──▶ select step ──▶ confirm step ──▶ action step ──▶ (완료/반복)
                                                    │
                                              ActionClient
                                              (httpx async)
                                                    │
...
- 전체 async 파이프라인 (Gateway → Engine → ActionClient → HTTP)
- PostgreSQL 세션 영속화 (WorkflowSessionStore)
- Per-workflow escape keywords
...
Planner (질문 분석 → 계획 수립)
    │
    ▼
Executor (Tool 실행 + 결과 수집)
    │
    ▼
Adaptive Retry (필요 시 재시도, max 2회)
    │
    ▼
Guardrail Regen (안전 검증 실패 시 재생성)
```
